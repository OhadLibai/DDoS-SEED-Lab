...
docker exec slow-headers-attacker bash -c 'grep -c " 01 " /proc/net/tcp' 2>/dev/null | \
...
docker exec apache-http2-victim bash -c 'ps aux | grep httpd | wc -l' 2>/dev/null | \
...
docker logs apache-http2-victim 2>&1 | tail -10 | grep "HTTP/2.0" | wc -l | \
...
docker logs apache-http2-victim 2>&1 | tail -20 | grep "POST" | wc -l | \
...
docker logs apache-http2-victim 2>&1 | tail -20 | grep "200" | wc -l | \
